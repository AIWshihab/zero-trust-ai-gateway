  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 780px) {
      ...
      /* Show inspector below chat on mobile */
      ...
      /* Sidebar nav link row — scroll horizontally so it doesn't push layout */
...
<body>
  ...
  <script>
    ...
    const linkConfig = [
      ...
      { label: "Compare", href: "/dashboard/models/compare", requiresAdmin: false },
      { label: "Security Tests", href: "/dashboard/security", requiresAdmin: false },
      { label: "SOC", href: "/dashboard/soc", requiresAdmin: true },
      { label: "Firewall", href: "/dashboard/firewall", requiresAdmin: true },
      { label: "Models", href: "/models-manager", requiresAdmin: false },
    ];
    ...
    function renderMessages() {
      ...
      if (!session.messages.length) {
        ...
        const emptyTitle = hasAny ? "Start A Protected Chat" : "No Models Available Yet";
        const emptyBody = hasAny
          ? "Choose a registered model, ask normally, and the gateway will keep the conversation screened with prompt guard, formal risk, trust scoring, and audit logging."
          : "No chat-capable models are currently visible to your account. Ask an admin to onboard or enable a model, then return here.";
        $("messages").innerHTML = `<div class="empty"><strong>${escapeHtml(emptyTitle)}</strong><span>${escapeHtml(emptyBody)}</span>${hasAny ? `<span class="muted">Runtime-ready models: ${readyCount}</span>` : ""}</div>`;
        return;
      }
    ...
    function renderQuickLinks() {
      ...
      $("quickLinks").innerHTML = visible.map((item) => `
        <a class="secondary" href="${item.href}">
          ${escapeHtml(item.label)}${item.requiresAdmin ? ' <span class="badge">Admin</span>' : ""}
        </a>
    ...
    function renderGatewayNotice(detail, severity = "warn") {
      ...
      panel.innerHTML = `
        ...
        <div class="flow"><b>Next:</b> ${escapeHtml(detail.suggested_fix || "Choose another model or ask an admin to check setup.")}</div>
      `;
    ...
    async function loadProfile() {
      ...
      currentUser = {
        is_admin: Boolean((data.user?.scopes || []).includes("admin")),
      };
    ...
    async function loadModels() {
      ...
      if (!readyModel) {
        ...
        renderGatewayNotice({
          title: "No Ready Models",
          explanation: "Models are registered, but none are ready for full inference yet.",
          suggested_fix: "Ask an admin to configure runtime credentials or enable a protected model."
        });
      }
      if (!models.length) {
