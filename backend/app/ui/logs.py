  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 920px) {
      ...
      /* 4 metrics → 2-column on tablet */
      .stats { grid-template-columns: repeat(2, 1fr); }
    }
    @media (max-width: 600px) {
      /* Filter controls wrap neatly */
      .panel .row { flex-wrap: wrap; gap: 6px; }
      .panel .row select,
      .panel .row input { flex: 1 1 120px; min-width: 0; }
      /* Card head: badge wraps below title */
      .card-head { flex-direction: column; align-items: flex-start; gap: 6px; }
      .card .row { flex-wrap: wrap; gap: 5px; }
    }
